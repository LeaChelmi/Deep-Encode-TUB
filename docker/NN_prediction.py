...
def runPredictionBasedOnOurDataSetNN(path):
    ...
    data = pd.read_csv(os.path.join(os.getcwd(), 'default_dataset.csv'))
    ...
    # Train the model
    model.fit(X, Y, epochs = ep, batch_size=batchSize, verbose=0)
    ...
    csv_file_path = os.path.join(path, 'predictions_NN.csv')
